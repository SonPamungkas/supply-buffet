...
namespace SupplyBuffetMod
{
    public static class SupplyRunWet
    {
        ...
        private static void HandleNavalResupply(FactionHQ hq, Unit requester)
        {
            ...
            try
            {
                ...
                if (bestLoadoutHelo != null || bestLoadoutVtol != null)
                {
                    ...
                    {
                        if (ab != null && !ab.disabled && ab.CurrentHQ == hq && IsBaseAllowed(ab))
                        {
                            spawnBase = ab;
                            break;
                        }
                    ...
                    if (spawnBase == null)
                    {
                        foreach (var ab in airbases)
                        {
                            if (ab != null && !ab.disabled && ab.CurrentHQ == hq && 
                    ...
                    if (isShip)
                    {
                        foreach (Weapon w in weaponStation.Weapons)
                        {
                            if (w != null)
                            {
                                ...
                                Plugin.Log.LogInfo($"[SupplyBuffetMod] ExpressRearm enabled for naval weapon '{w.name}' on '{__instance.unitName}'");
                            }
                        }
                    }
                }
